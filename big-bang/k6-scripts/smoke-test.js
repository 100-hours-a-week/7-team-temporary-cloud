/**
 * K6 스모크 테스트 (Smoke Test)
 *
 * ============================================================================
 * 스모크 테스트란?
 * ============================================================================
 *
 * 최소한의 부하로 시스템의 기본 기능이 정상 동작하는지 확인하는 테스트입니다.
 *
 * 왜 필요한가?
 * 1. 배포 후 빠른 검증
 *    - 새 버전 배포 후 기본 기능이 동작하는지 1-2분 내에 확인
 *    - CI/CD 파이프라인에 통합하여 자동 검증
 *
 * 2. 테스트 스크립트 검증
 *    - 본격적인 부하 테스트 전 스크립트 오류 발견
 *    - API 응답 형식 변경 등으로 인한 스크립트 수정 필요성 확인
 *
 * 3. 환경 설정 검증
 *    - 테스트 환경의 네트워크 연결, 인증 설정 등 확인
 *    - 테스트 사용자 계정이 유효한지 확인
 *
 * 언제 실행하는가?
 * - 매 배포 후 자동 실행 (CI/CD)
 * - 본격적인 부하 테스트 전 사전 검증
 * - 개발 중 API 변경 후 동작 확인
 *
 * 실행 방법:
 * k6 run smoke-test.js
 * K6_BASE_URL=https://staging.example.com k6 run smoke-test.js
 */

import { sleep } from 'k6';
import {
    SMOKE_TEST_STAGES,
    COMMON_THRESHOLDS,
} from './config.js';
import {
    signup,
    logout,
    getProfile,
    getSchedulesByDate,
    healthCheck,
    thinkTime,
} from './helpers.js';
import { createReportOutput } from './report-generator.js';

// ============================================================================
// 테스트 설정
// ============================================================================

export const options = {
    /**
     * 스모크 테스트 단계 설정
     *
     * 30s: 1 VU → 1m: 5 VU → 30s: 0 VU
     *
     * 왜 이렇게 설정하는가?
     * - 최소한의 부하로 빠르게 검증 (총 2분)
     * - 1명으로 시작하여 기본 동작 확인
     * - 5명으로 증가하여 동시성 기본 테스트
     */
    stages: SMOKE_TEST_STAGES,

    /**
     * 스모크 테스트용 임계값
     *
     * 일반 테스트보다 엄격한 기준 적용:
     * - 기본 기능이므로 빠른 응답 필수
     * - 실패율 0% 목표 (0.1% 허용)
     */
    thresholds: {
        http_req_duration: ['p(95)<1000', 'p(99)<2000'],
        http_req_failed: ['rate<0.001'], // 0.1% 미만
        checks: ['rate>0.99'], // 99% 이상의 체크 통과
    },
};

// ============================================================================
// 메인 테스트 함수
// ============================================================================

/**
 * 스모크 테스트 시나리오
 *
 * 테스트 플로우:
 * 1. 헬스체크 - 서버 가용성 확인
 * 2. 회원가입 - 신규 사용자 등록 (고유 이메일 자동 생성)
 * 3. 프로필 조회 - 인증된 API 호출 확인
 * 4. 스케줄 조회 - 핵심 비즈니스 API 확인
 * 5. 로그아웃 - 세션 정리 확인
 *
 * 왜 회원가입을 사용하는가?
 * - 매 테스트마다 새로운 사용자 생성으로 독립적인 테스트 보장
 * - 이메일은 타임스탬프 + 랜덤 문자열로 중복 방지
 * - 실제 신규 사용자 플로우 테스트
 */
export default function () {
    // Step 1: 헬스체크
    const isHealthy = healthCheck();
    if (!isHealthy) {
        console.error('Server health check failed - aborting test');
        return;
    }

    thinkTime(0.5, 1);

    // Step 2: 회원가입 (고유 이메일 자동 생성)
    const signupResult = signup();

    if (!signupResult) {
        console.error(`VU ${__VU}: Signup failed - aborting iteration`);
        return;
    }

    const { accessToken } = signupResult;

    thinkTime(0.5, 1);

    // Step 3: 프로필 조회
    const profile = getProfile(accessToken);
    if (!profile) {
        console.warn(`VU ${__VU}: Get profile failed`);
    }

    thinkTime(0.5, 1);

    // Step 4: 오늘 스케줄 조회
    getSchedulesByDate(accessToken);

    thinkTime(0.5, 1);

    // Step 5: 로그아웃
    logout(accessToken);

    // 다음 반복 전 짧은 대기
    sleep(1);
}

// ============================================================================
// 라이프사이클 훅
// ============================================================================

/**
 * 테스트 시작 전 실행
 * 전체 테스트에서 한 번만 실행됨
 */
export function setup() {
    console.log('========================================');
    console.log('🔥 Smoke Test Started');
    console.log('========================================');
    console.log('Mode: 회원가입 기반 테스트 (고유 이메일 자동 생성)');
    console.log('');

    // 서버 가용성 사전 확인
    const isHealthy = healthCheck();
    if (!isHealthy) {
        throw new Error('Server is not responding - cannot proceed with test');
    }

    return { startTime: new Date().toISOString() };
}

/**
 * 테스트 종료 후 실행
 */
export function teardown(data) {
    console.log('');
    console.log('========================================');
    console.log('✅ Smoke Test Completed');
    console.log('========================================');
    console.log(`Started: ${data.startTime}`);
    console.log(`Finished: ${new Date().toISOString()}`);
}

/**
 * 테스트 결과 보고서 자동 생성
 */
export function handleSummary(data) {
    return createReportOutput(data, 'smoke-test');
}
